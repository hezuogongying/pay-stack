...
from gopay.config import QQConfig
...
class QQClient(PaymentClient):
    ...
    def __init__(self, config: QQConfig):
        ...
        :param config: QQ支付配置
        """
        if not isinstance(config, QQConfig):
            raise ValueError("配置必须是 QQConfig 类型")
