...
class LakalaConfig(PaymentConfig):
    """
    拉卡拉支付配置
    遵循开闭原则 - 继承基类扩展拉卡拉特有配置
    """

    # 拉卡拉支付网关
    gateway_url: str = "https://sds.lakala.com/api"
...
class LakalaClient(PaymentClient):
    """
    拉卡拉支付客户端
    支持拉卡拉支付的各种支付方式
    """

    def __init__(self, config: LakalaConfig):
        """
        初始化拉卡拉支付客户端
        :param config: 拉卡拉支付配置
        """
        if not isinstance(config, LakalaConfig):
            raise ValueError("配置必须是 LakalaConfig 类型")

        super().__init__(config)
        self.config: LakalaConfig = config
    ...
    def _sign_params(self, params: Dict[str, Any]) -> str:
        ...
        if not self.config.api_key:
            raise SignError("API密钥未配置", "LAKALA")
    ...
    def _build_common_params(self) -> Dict[str, Any]:
        """构建公共参数"""
        return {
            "merchant_id": self.config.mch_id or self.config.app_id,
            "terminal_id": self.config.terminal_id,
            "version": self.config.version,
            "charset": self.config.charset,
            "random_str": self._generate_nonce_str(),
        }
